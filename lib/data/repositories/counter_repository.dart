...
class CounterRepository {
  ...
  Future<Counter> _mapRow(CounterRow r) async {
    final alerts = await db.getAlertsForCounter(r.id);
    
    // Fallback: se não houver alertas na nova tabela mas houver alertOffset na coluna antiga
    List<int> alertOffsets = alerts.map((a) => a.offsetMinutes).toList();
    if (alertOffsets.isEmpty && r.alertOffset != null) {
      alertOffsets = [r.alertOffset!];
    }
  ...
  Future<List<Counter>> all() async {
    final rows = await db.getAllCounters();
    final counters = <Counter>[];
    for (final r in rows) {
      counters.add(await _mapRow(r));
    }
    return counters;
  }
  ...
  Future<int> createWithHistory(Counter c) async {
    ...
    for (final offset in c.alertOffsets) {
      await db.insertAlert(CounterAlertsCompanion.insert(
        counterId: id,
  ...
  Future<bool> updateWithHistory(Counter c) async {
    ...
    if (current != null) {
      final snapshot = _buildSnapshotFromCounter(await _mapRow(current));
      await db.insertHistory(CounterHistoryCompanion.insert(
    ...
    // Atualizar alertas: deletar os antigos e inserir os novos
    await db.deleteAlertsForCounter(c.id!);
    for (final offset in c.alertOffsets) {
      await db.insertAlert(CounterAlertsCompanion.insert(
        counterId: c.id!,
        offsetMinutes: offset,
